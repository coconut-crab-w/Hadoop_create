...
> 2. 上传Zookeeper压缩包的路径：`/usr/local/zookeeper`
> 3. 将Zookeeper压缩包解压到指定目录下：`tar -xzvf apache-zookeeper-3.8.5-bin.tar.gz -C /usr/local/zookeeper`
> 4. 配置环境变量：
>    1. 打开环境变量配置文件：`sudo vim /etc/profile`
>    2. 文件末尾添加内容：`export ZOOKEEPER_HOME=/usr/local/zookeeper/apache-zookeeper-3.8.5-bin`
